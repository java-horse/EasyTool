...
public abstract class AbstractAnnotationHolder implements AnnotationHolder {
    ...
    protected void addInfoToNotes(CommentInfo commentInfo) {
        ...
        if (StringUtils.isNotBlank(notes)) {
            notes += Constants.BREAK_LINE;
        }
        String example = commentInfo.getExample("");
        if (StringUtils.isNotBlank(example)) {
            notes += "示例值为 " + example + Constants.BREAK_LINE;
        }
        ...
        if (hasLengthAnnotation) {
            ...
            if (min != null) {
                notes += "最小长度为 " + min + Constants.BREAK_LINE;
            }
            if (max != null) {
                notes += "最大长度为 " + max + Constants.BREAK_LINE;
            }
        }
        Number min = null;
        Number max = null;
        ...
        if (min != null) {
            notes += "最小值为 " + min + Constants.BREAK_LINE;
        }
        if (max != null) {
            notes += "最大值为 " + max + Constants.BREAK_LINE;
        }
        if (notes.endsWith(Constants.BREAK_LINE)) {
            notes = notes.substring(0, notes.length() - Constants.BREAK_LINE.length());
        }
