...
public abstract class AbstractIconType extends IconType {
    ...
    protected AbstractIconType(@NotNull String name, @NotNull String selectSuffix, @NotNull String suffix) {
        ...
        for (HttpMethod httpMethod : HttpMethod.values()) {
            String iconPath = String.format("icons/restful/%s/%s.%s", name, httpMethod.name(), suffix);
            String iconSelectPath = String.format("icons/restful/%s/%s%s.%s", name, httpMethod.name(), selectSuffix, suffix);
